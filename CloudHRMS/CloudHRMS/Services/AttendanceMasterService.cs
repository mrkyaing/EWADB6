...
namespace CloudHRMS.Services
{
    public class AttendanceMasterService : IAttendanceMasterService
    {
        ...
        public void Delete(DateTime fromDate, DateTime toDate, string departmentId, string employeeId)
        {
            try
            {
                var collectAttendanceMasters = _unitOfWork.AttendanceMasterRepository.GetAll(w => (w.AttendanceDate.Date >= fromDate.Date && w.AttendanceDate.Date <= toDate.Date) && w.DepartmentId == departmentId && w.EmployeeId == employeeId).ToList();
